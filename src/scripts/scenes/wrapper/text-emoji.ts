...
export class TextEmoji {
    ...
    static create(args: (string | PIXI.Texture)[], fontSize: number = DEFAULT_FONT_SIZE): TextEmoji {
        ...
        return new TextEmoji(components, fontSize);
    }
    ...
    setContent(content: IComponent[], fontSize: number) {
        ...
        const style = new PIXI.TextStyle({ fontSize: fontSize, fill: '#FFFFFF' });
        ...
        for (let i = 0; i < tuples.length; i++) {
            ...
            const effect = maxHeight / (metrics.height - descent);
            if (effect !== 1) {
                metrics.width *= effect;
                metrics.height *= effect;
                visual.scale = new PIXI.Point(effect, effect);
            }
    ...
    destroy() {
        this.visual.removeChildren().forEach(child => child.destroy());
    }
}